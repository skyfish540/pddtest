...
public class TestSliderMove {
    ...
    public static void invoke() throws IOException, InterruptedException {
        ...
        //通过[class=gt_slider_knob gt_show]获得 ”滑动按钮”
        Thread.sleep(10000L);
        WebElement moveElemet = driver.findElement(By.xpath("//*[@id='slide-button']"));
        Thread.sleep(2000L);
        int i = 0;
        while (i++ < 15){
            int distance = getMoveDistance(driver); // 获得需要移动的距离
            move(driver, moveElemet, distance - 6); //开始移动滑块
...
//            printLocation(element);
    ...
    public static int getMoveDistance(WebDriver driver) throws IOException {
        /*String pageSource = driver.getPageSource(); //获取页面内容
        ...
        BufferedImage bgBI = ImageIO.read(new File("F:/pddLogger/img/2020-10-13/2.png"));
    ...
    private static String getFullImageUrl(String pageSource){
        ...
        String style = document.select("[class=slider-item]").first().attr("style");
